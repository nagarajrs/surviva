...
| Detection | `surviva daemon` | Polls IMDSv2 for the Spot rebalance recommendation (soft, early) and interruption notice (hard, ~2min). Either fires the checkpoint pipeline for every tracked job. |
