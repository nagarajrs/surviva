...
Whatever long-running command the user is protecting gets frozen in place — its entire memory and state captured, like a snapshot — so it can be resumed later exactly where it stopped, not restarted from scratch. From that same moment, the machine also stops accepting brand-new jobs to protect — one that started after the warning already arrived would have no realistic chance of being saved before the machine is actually taken away, so trying to protect it would just be a false promise. The command itself still runs; it's just told plainly that this particular run won't survive an interruption.
