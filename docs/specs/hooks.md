...
feature spanning `store`, `daemon`, and `surviva-cli`. This spec exists
because it never got a dedicated design doc of its own, and adds one piece
of real validation that was missing.

## Objective

An escape hatch for jobs CRIU can't handle (GPU state, certain sockets —
see [../LIMITATIONS.md](../LIMITATIONS.md)) or where a user simply wants
their own application-specific save/restore mechanism instead of a generic
...
- `<checkpoint-dir>` is the job's `store.Job.CheckpointDir` (see
  [store.md](store.md)) — the hook can read/write whatever files it wants
  there, same directory every time for that job.
...
no new tests needed there. See [../../hooks/README.md](../../hooks/README.md)
for how a hook author tests their own script standalone, without any of
this.
...
- `hooks/examples/*.sh` (see [../../hooks/README.md](../../hooks/README.md)) work end-to-end via
