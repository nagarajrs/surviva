...
audit logging (`component: "cli"`) per [audit-log.md](audit-log.md)'s
division of responsibility.

`run` uses `internal/procattr` (`New() *syscall.SysProcAttr`, `Setsid:
true`) to start the tracked child as its own session/process-group leader
(PGID == PID), and `internal/fdguard` (`CloseInherited()`) to close this
process's own inherited fds before starting the child, so a stray
pty/socket fd can't leak in and break CRIU later. `internal/sigset`
(`TermSignals()`) tells the `daemon` subcommand which OS signals mean
"shut down." `internal/procsignal.KillGroup` forwards Ctrl-C/SIGTERM to
the tracked child while `run` waits on it (also used by `daemon` for
`Cancel`).
...
and exit codes `0`/`1`/`2` (success/failure/usage).
...
3. Forward SIGINT/SIGTERM to the child's process group as SIGTERM: the
   ...
   and a backgrounded shell script ignores SIGINT for itself regardless —
   a POSIX rule, not a surviva quirk.
...
**No `-priority` flag.** `store.Job` carries no priority field (checkpoint
fan-out ordering isn't a concern this phase — see Open Questions). Add
both together if it turns out to matter.
...
- `Command`/`WorkDir`: best-effort from `/proc/<pid>/cmdline` and
  `/proc/<pid>/cwd` (Linux only — surviva is a Linux-only production tool).
  Empty if unreadable; `store` doesn't require them non-empty (per
  [store.md](store.md)'s note on this exact case).
...
(`store.IsTerminal` tells `jobDuration` which) — see
[store.md](store.md)'s job audit trail section for why this isn't a
`store` column.
...
restore attempts, interruption detection), per [audit-log.md](audit-log.md).
