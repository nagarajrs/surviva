...
this module's call: a Unix domain socket speaking newline-delimited JSON.

Session detachment (`internal/procattr`) and closing inherited fds
(`internal/fdguard`) are `surviva-cli run`-time concerns, not `daemon`'s —
whoever `exec`s the tracked process needs them, and that's the CLI. They
belong in `surviva-cli`'s own spec.

`daemon` owns CRIU dump/restore (`internal/criu`, `internal/checkpoint`,
`internal/resume`), the cloud-signal poller (`internal/imds`, wrapped
behind the `Provider` interface below so Azure/GCP can be added later as
siblings, not a rewrite), and process signaling for `Cancel`
(`internal/procsignal.KillGroup`). Job ids are Slurm-style sequential
integers, assigned by `store.Insert`'s `INTEGER PRIMARY KEY AUTOINCREMENT`
column — see [store.md](store.md).
...
`aws.New(...)` is the only implementation today; `config.CloudProvider ==
"aws"` selects it (already the only value `config.Load` accepts — see
[config.md](config.md)). Azure/GCP become new packages implementing the
same interface once `config` accepts those values too; no change to
`daemon`'s own logic when that happens.
...
Deliberately lighter than surviva itself owning a full Step-Functions-based
...
This is the only feature needing the real AWS SDK
...
`cmd/surviva/daemon.go`'s wiring — the standard instance-role credential
chain.
...
|---|---|---|---|---|
| `MaxConcurrentCheckpoints` | positive int | no | `runtime.NumCPU()` | Bound on simultaneous CRIU dumps during a fan-out (dumping many large process trees fully in parallel can starve disk/CPU badly enough that none finish in time). |
...
All four below share one rule: a job's `store` status arbitrates races
between daemon-side transitions and whatever the CLI/interruption path is
doing concurrently. Every transition goes through
`store.UpdateStatus`/`UpdatePID`, which already reject an invalid `from`
state — `daemon` doesn't need its own separate guard on top, just surface
`store`'s error back over IPC.
...
[surviva-cli.md](surviva-cli.md)) for a CLI-driven transition, or the
literal string `"daemon"` for one the daemon makes on its own (the
interruption fan-out). `store` records it as a `job_history` row in the
same transaction as the status update — see [store.md](store.md)'s job
audit trail section. Omitted below for brevity; assume every
`UpdateStatus`/`UpdatePID` call takes it.
...
- If current status isn't `RUNNING` anymore, this is a no-op (not an
  error) — a real race: the checkpoint subsystem may have already moved
  the job past `RUNNING` (CRIU's dump stops the process, which can race
  the CLI noticing its child exited).
...
the request):
1. Refused if the interruption latch (below) is set — a job registered
   after a signal already fired has no realistic path to being saved.
2. `id, err := store.Insert(Job{..., CheckpointDir: req.CheckpointDir, Status: RUNNING, Owner: req.RequestedBy})`
   — `store` itself assigns `id` (sequential, see [store.md](store.md)); if the
...
On `onSignal`, latch `interrupted` (an `atomic.Bool`, refusing new
`Register` calls from that point on), list every `RUNNING` job from
`store`, and run the Checkpoint flow above for each, capped at
`MaxConcurrentCheckpoints` concurrent via a bounded worker pool. Runs
independently of any client connection — it's driven by the `Provider`,
not by `dispatch`.
...
CRIU itself needs a real Linux box (real infra over mocks, project-wide);
that part is manual/integration, not `go test ./...`. What *is*
unit-testable here, against a temp-file `store` + `auditlog` and a fake
`Provider` that never fires:
