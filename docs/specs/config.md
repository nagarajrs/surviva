...
| `CheckpointBaseDir` | path | yes | Base dir `daemon` computes each job's checkpoint dir under (`<base>/<job-id>`). |
| `DBType` | string, one of `sqlite` (default), `mysql` | no | Selects `store`'s backend — see [store.md](store.md). Absent means `sqlite`, so every `surviva.conf` written before this directive existed keeps working unchanged. |
