...
## 12. One shared logbook, not one per machine

Once several parts of the system needed somewhere to write down what happened, the choice was one shared, central logbook per deployment rather than a separate one on every single machine. This matters specifically because the whole point of the automatic-replacement system is that a machine involved in a real failure might already be gone by the time anyone goes looking — a per-machine logbook would disappear right along with it. Setting this up surfaced a real, easy-to-miss gotcha: the cloud's remote-command tool needed permission not just to *write* to the logbook, but also to *create* it — even though it already existed — and without that specific permission, it silently failed to write anything at all, with no error shown anywhere obvious. The fix was granting that one extra permission; the lesson, confirmed for real, was to check a tool's own local activity log when something that should be showing up simply isn't.

## 13. Manual pause reuses the exact same save logic, not a copy of it

When a person can now trigger a save by hand instead of waiting for an interruption warning, the temptation is to write a second, simpler version of "save this program" just for that button. Surviva doesn't do that — pressing the button runs the identical save logic the automatic system already uses, with one extra on/off switch for "keep this one on local disk only, don't send it anywhere else." Two separate copies of something this important would only ever drift apart over time, catching a fix in one but not the other. The one risk this does introduce — a person manually saving a program at the exact moment the automatic system also decides to save it — is handled by simply refusing the manual request once a real interruption warning has already arrived, since the automatic system is already covering every program at that point anyway.

## 14. Bringing a program back on the same machine doesn't need any of the cloud-recovery machinery

Surviva already had a way to bring a saved program back to life — but only as part of the full disaster-recovery story, which assumes a brand-new replacement machine and needs a shared tracking record in the cloud to know what to fetch and from where. That's the wrong tool for someone who paused a program by hand and just wants it back on the very same machine, especially if they chose to keep that save purely local (no cloud tracking record was ever created for it). So a separate, much simpler bring-it-back command was added, one that only ever looks at this machine's own local save folder and never talks to the cloud at all. It picks up right where the program was paused — same job, not a brand new one — but it's a genuinely different situation from full disaster recovery, and it does nothing to help if the machine itself is lost first, which is exactly the case the original, cloud-backed way of bringing a program back still exists to handle.

---

These twelve decisions aren't the complete list of everything considered while building Surviva, but they're the clearest illustrations of a consistent pattern: prefer the documented, simple, well-tested path; verify assumptions against reality whenever possible; and let genuine failures — not guesses — drive what gets fixed and how.
