...
## ADR-16: One shared CloudWatch log group, not per-instance groups or S3-only SSM output

**Context.** Troubleshooting a real restore failure by hand (SSM into a replacement instance, `tail` its `restore.log`) works but doesn't scale to "the instance a failure happened on may already be gone" — exactly what makes the sandbox's own destroy flow correct also makes after-the-fact debugging hard. Needed: somewhere failures are visible without depending on a specific instance still existing.

**Decision.** One log group per deployment (`/surviva/<name_prefix>`, `infra/terraform/cloudwatch.tf`), not one per instance. Two things flow into it: the daemon's own ongoing log, via the CloudWatch Agent (installed and configured once, during the AMI bake, tailing a plain file the systemd unit now writes to instead of journal — journal export would have meant either the agent's less-common `systemd` log source or a separate export step, both more moving parts for the same result); and each restore attempt's own SSM command output, via `SendRestoreCommand`'s native `CloudWatchOutputConfig` rather than the alternative `OutputS3*` fields — CloudWatch was already needed for the daemon's own log, and a single destination beats correlating two.

Rejected: per-instance log groups (no benefit here — nothing queries across a fleet of many concurrently-running instances the way a larger production deployment might, and one group is simpler to grant IAM for and to `aws logs tail`); SSM's S3 output path (would need its own bucket, and still wouldn't cover the daemon's own independent, ongoing log).

**A real IAM gap found deploying this, not anticipated in advance.** Granting the instance role the correctly-scoped stream-level actions (`CreateLogStream`/`PutLogEvents`/`DescribeLogStreams` against `<log-group-arn>:*`) was not sufficient for SSM's `CloudWatchOutputConfig`: it unconditionally attempts `logs:CreateLogGroup` against the **bare** group ARN first, even when the group already exists, and that resource pattern doesn't match a `:*`-suffixed grant (the literal `:` before the wildcard has nothing to match in the bare ARN). The command itself still reports success — the failure is only visible in the instance's own `/var/log/amazon/ssm/amazon-ssm-agent.log`. Fixed by granting `CreateLogGroup` against the bare ARN too (see limitation 22). The CloudWatch Agent path never hit this, since it only ever calls the stream-level actions itself.

**Consequences.** `surviva-daemon.service`'s `StandardOutput`/`StandardError` changed from `journal` to `append:/var/log/surviva-daemon.log` — anyone reading the daemon's log directly on an instance (rather than via CloudWatch) now reads that file, not `journalctl -u surviva-daemon`. The instance role gained `logs:*` actions scoped tightly to this one log group's two ARN forms; no change to the state machine role (SSM's CloudWatch delivery runs as the target instance, not as the orchestrator).

## ADR-17: Manual pause reuses `checkpointJob` with a per-call storage override, not a separate code path

**Context.** `surviva pause <job-id>` needed to let a person trigger the exact same checkpoint (CRIU dump or hook, then optional S3/EBS durability) that an interruption notice already triggers automatically, plus a way to force that one checkpoint onto local disk only regardless of the daemon's own S3/EBS configuration.

**Decision.** `checkpointJob(j job.Job)` became `checkpointJob(j job.Job, forceLocal bool) error`: the two storage branches (`d.ebsVolumeID != ""`, `d.remoteEnabled()`) each gained a `!forceLocal` guard, and the function now returns its terminal error (previously only logged) so `ActionPause`'s synchronous dispatch handler can report it back over IPC. `handleInterruption`'s fan-out calls it with `forceLocal=false` and ignores the returned error, since it's already logged the same way it always was. Rejected: a second, parallel "manual checkpoint" implementation — that would have meant two places that could drift on CRIU flags, EBS start-record timing, or status transitions, for behavior that's supposed to be identical except for the trigger and the storage override.

**Consequences.** A manual pause and an interruption-triggered checkpoint are now guaranteed to behave identically (same timeout, same EBS "start" record ordering, same status transitions) except for the one flag. The daemon refuses `ActionPause` outright once `d.interrupted` has latched, rather than letting it interleave with `handleInterruption`'s own fan-out over the same job set — a manual pause targets exactly one job, but two `criu dump`s racing the same pid would be a real hazard, not just wasted work.

## ADR-18: `surviva resume` is same-instance/local-disk-only; cross-instance recovery stays on `surviva restore`

**Context.** A job paused with `surviva pause -local` has, by design, no S3 object, no EBS durability record, and no DynamoDB row — `surviva restore` cannot resume it (it hard-requires `-dynamodb-table` and reads the job's state entirely from that record). But the whole point of a manual pause is being able to bring the job back manually too.

**Decision.** Added `surviva resume <job-id>` as a distinct command rather than teaching `restore` a local mode. It resumes straight from `checkpoint.Dir(d.checkpointDir, jobID)` — the local dump every checkpoint path writes first, regardless of storage backend — using the job's own row in the local SQLite store (already retained, unmodified, since `checkpointJob` never deletes it on success) for `HookResume`. On success it updates that same row in place (new pid/pgid, status back to `RUNNING`) instead of registering a new job id, since same-instance resume has no reason to mint a new identity for a job the user already knows by its existing one. Rejected: extending `restore` with a `-local` flag — `restore`'s whole shape (load AWS config, read a DynamoDB record, branch on `StorageType`) is irrelevant and actively wrong for a same-instance resume with no such record to read.

**Consequences.** `surviva resume` needs no AWS credentials or configuration at all, and works for *any* still-present local checkpoint — including ones made the normal (S3/EBS) way, as long as nobody has already restored them elsewhere and the local images haven't been cleaned up. It is not a substitute for `restore`: it cannot help once the instance that made the checkpoint is gone, which is exactly the case `restore` (and the default, non-`-local` storage mode) exists to cover — see limitation 23.
