...
# See SPEC-store.md for the full backend design.
#DBType=mysql
#DBHost=127.0.0.1
#DBPort=3306
#DBUser=surviva
...
# SPEC-daemon.md for the JSON payload schema. Requires the IAM permissions
# in step 4 below.
#NotifyTargetType=lambda
#NotifyTargetARN=arn:aws:lambda:us-east-1:123456789012:function:my-fn
```

See `SPEC-config.md` for the complete directive reference (every key,
type, and validation rule).
...
Logs go to the journal (systemd's default) — no CloudWatch integration
exists in this redesign, unlike `legacy`. Switching to file-based output
later (e.g. `StandardOutput=append:/var/log/surviva-daemon.log` in the
unit) is a one-line change if that comes back.
