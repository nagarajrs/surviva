...
Real gotchas found while building/testing this redesign, written down so
they don't get rediscovered from scratch. Mirrors `legacy/docs/limitations`
in spirit, scoped to the new architecture.
...
This is a deliberate, long-standing design assumption, not a regression:
`internal/criu/criu.go`'s own comment (carried over from `legacy` unchanged)
says a tracked child needs no shell/tty to reattach to on dump or restore,
and `--shell-job` is deliberately not used to work around it.
...
# step (setsid satisfies join's other requirement too -- see SPEC-surviva-cli.md)
...
straight to its own by default (matching `legacy`), rather than defaulting
to `/dev/null` to avoid this footgun automatically. Revisit if this proves
too easy to hit by accident in practice.
