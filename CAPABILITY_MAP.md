...
| `surviva-cli` | `surviva run`/`pause`/`resume`/`list`/`show`/`cancel`/`prune`/`join <pid>`, plus `surviva daemon` (wires the `daemon` module into a running process). | `daemon`, `audit-log` | [SPEC-surviva-cli.md](SPEC-surviva-cli.md) | Spec written, pending review |
