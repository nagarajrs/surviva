...
| `surviva-cli` | `surviva run`/`pause`/`resume`/`list`/`show`/`cancel`/`prune` (and `join <pid>` to adopt an already-running external process into tracking). Talks to the running `daemon` for anything job-related. | `daemon`, `audit-log` | not yet written | Not started |
