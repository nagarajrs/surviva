...
| `surviva-cli` | `surviva run`/`pause`/`resume`/`list`/`show` (and likely `cancel`). Talks to the running `daemon` for anything job-related. | `daemon`, `audit-log` | not yet written |
